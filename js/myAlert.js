(function () {
    $.extend({
        ...
        myLight:function(options){
            ...
            var top = $(window).height() * 0.3;
            $('body').append('<div class="myModa"><div class="myAlertBox mylight" style="margin-top:' + top + 'px">' + option.message+'</div></div>');
            $('.mylight').click(function () {
                option.callback();
                // $('.myModa').remove();
            })
        },
        myQuestion: function (options) {//参数格式{title:'Title',message:'message',callback:function(){alert('callback')}}or"需要提示的话"$.myConfrim()
            var option = {
                title: "提示",
            ...
            var top = $(window).height() * 0.3;
            $('body').append('<div class="myModa"><div class="myAlertBox" style="margin-top:' + top + 'px">'  + '<p>' + option.message + '</p><div class="col2"><div class="col" style="margin-right: 20px;"><div class="btn exit">' + option.btn1+'</div></div><div class="col"><div class="btn sure">'+ option.btn2+'</div></div></div></div></div>');
            $('.btn.exit').click(function () {
                $('.myModa').remove();
                option.callback();
            })
